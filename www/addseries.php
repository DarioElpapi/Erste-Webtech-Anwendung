...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST["title"]) && isset($_POST["erscheinungsjahr"]) && isset($_POST["genre"]) && isset($_POST["staffeln"]) && isset($_POST["imdb_link"])) {
        ...
        $staffeln = intval($_POST["staffeln"]); // Stelle sicher, dass es sich um eine ganze Zahl handelt
        $imdb_link = mysqli_real_escape_string($conn, $_POST["imdb_link"]);

        // Füge den Film zur Datenbank hinzu
        $sql = "INSERT INTO serien (title, erscheinungsjahr, genre, staffeln, link) VALUES ('$title', $erscheinungsjahr, 1,$staffeln, '$imdb_link')";

        if ($conn->query($sql) === TRUE) {
            // Film erfolgreich hinzugefügt, jetzt fügen wir die Verknüpfung zwischen Film und Genre hinzu
            $last_series_id = $conn->insert_id; // ID des zuletzt eingefügten Films abrufen

            // Füge die Verknüpfung in die Tabelle movie_genres ein
            $update_sql = "UPDATE serien SET genre = $genre_id WHERE id = $last_series_id";
            if ($conn->query($update_sql) === TRUE) {
                echo "serie erfolgreich zur Datenbank hinzugefügt.";
                $user_email = $_SESSION["email"];
                $favorite_sql = "INSERT INTO user_serien (email, serie) VALUES ('$user_email', $last_series_id)";
                if ($conn->query($favorite_sql) === TRUE) {
                    echo "serie wurde zur Favoritenliste hinzugefügt.";
                }
            } else {
                echo "Fehler beim Hinzufügen des serie: " . $conn->error;
            }
        } else {
            echo "Fehler beim Hinzufügen des serie: " . $conn->error;
        }
